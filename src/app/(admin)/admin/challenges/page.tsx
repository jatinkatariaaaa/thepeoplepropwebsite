...
export default function ChallengesAdminPage() {
  ...
  if (loading && programs.length === 0) return <div className="p-10 animate-pulse bg-[var(--dash-canvas)] rounded-none h-40" />;
    ...
    <div className="space-y-8 max-w-6xl mx-auto relative">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-none shadow-xl w-full max-w-lg overflow-hidden max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSave} className="p-4 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input required disabled={!!editingProgram} type="text" value={formData.key} onChange={e => setFormData({...formData, key: e.target.value.toLowerCase()})} className="w-full rounded-none border border-[var(--dash-hairline)] bg-white px-3 py-2 text-sm text-ink outline-none transition-colors focus:border-ink-400 bg-gray-50" />
                </div>
                <div>
                  <label className="mb-1 block text-xs font-medium text-ink-700">Short Label</label>
                  <input required type="text" value={formData.short_label} onChange={e => setFormData({...formData, short_label: e.target.value})} className="w-full rounded-none border border-[var(--dash-hairline)] bg-white px-3 py-2 text-sm text-ink outline-none transition-colors focus:border-ink-400" />
                </div>
              </div>
              <div>
                <label className="mb-1 block text-xs font-medium text-ink-700">Full Label</label>
                <input required type="text" value={formData.label} onChange={e => setFormData({...formData, label: e.target.value})} className="w-full rounded-none border border-[var(--dash-hairline)] bg-white px-3 py-2 text-sm text-ink outline-none transition-colors focus:border-ink-400" placeholder="e.g. 1-Step Evaluation" />
              </div>
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <label className="mb-1 block text-xs font-medium text-ink-700">Profit Target</label>
                  <input type="text" value={formData.profit_target} onChange={e => setFormData({...formData, profit_target: e.target.value})} className="w-full rounded-none border border-[var(--dash-hairline)] bg-white px-3 py-2 text-sm text-ink outline-none transition-colors focus:border-ink-400" placeholder="10%" />
                </div>
                <div>
                  <label className="mb-1 block text-xs font-medium text-ink-700">Max Drawdown</label>
                  <input type="text" value={formData.max_drawdown} onChange={e => setFormData({...formData, max_drawdown: e.target.value})} className="w-full rounded-none border border-[var(--dash-hairline)] bg-white px-3 py-2 text-sm text-ink outline-none transition-colors focus:border-ink-400" placeholder="6%" />
                </div>
                <div>
                  <label className="mb-1 block text-xs font-medium text-ink-700">Daily Drawdown</label>
                  <input type="text" value={formData.daily_drawdown} onChange={e => setFormData({...formData, daily_drawdown: e.target.value})} className="w-full rounded-none border border-[var(--dash-hairline)] bg-white px-3 py-2 text-sm text-ink outline-none transition-colors focus:border-ink-400" placeholder="3%" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="mb-1 block text-xs font-medium text-ink-700">Profit Split %</label>
                  <input type="number" value={formData.profit_split} onChange={e => setFormData({...formData, profit_split: Number(e.target.value)})} className="w-full rounded-none border border-[var(--dash-hairline)] bg-white px-3 py-2 text-sm text-ink outline-none transition-colors focus:border-ink-400" />
                </div>
                <div>
                  <label className="mb-1 block text-xs font-medium text-ink-700">Evaluation Phases</label>
                  <select value={formData.phases} onChange={e => setFormData({...formData, phases: Number(e.target.value)})} className="w-full rounded-none border border-[var(--dash-hairline)] bg-white px-3 py-2 text-sm text-ink outline-none transition-colors focus:border-ink-400 bg-white">
              ...
              <div className="grid grid-cols-1 gap-4 pt-4 border-t">
                ...
                <div>
                  ...
                  <select value={formData.phase_1_rule_id} onChange={e => setFormData({...formData, phase_1_rule_id: e.target.value})} className="w-full rounded-none border border-[var(--dash-hairline)] bg-white px-3 py-2 text-sm text-ink outline-none transition-colors focus:border-ink-400 bg-white text-sm">
                ...
                <div>
                  ...
                  <select value={formData.phase_2_rule_id} onChange={e => setFormData({...formData, phase_2_rule_id: e.target.value})} className="w-full rounded-none border border-[var(--dash-hairline)] bg-white px-3 py-2 text-sm text-ink outline-none transition-colors focus:border-ink-400 bg-white text-sm">
                    <option value="">No Rules Selected</option>
                    {rules.map(r => <option key={r.id} value={r.id}>{r.name}</option>)}
                  </select>
                </div>
                <div>
                  ...
                  <select value={formData.phase_3_rule_id} onChange={e => setFormData({...formData, phase_3_rule_id: e.target.value})} className="w-full rounded-none border border-[var(--dash-hairline)] bg-white px-3 py-2 text-sm text-ink outline-none transition-colors focus:border-ink-400 bg-white text-sm">
                    <option value="">No Rules Selected</option>
                    {rules.map(r => <option key={r.id} value={r.id}>{r.name}</option>)}
                  </select>
                </div>
                <div>
                  ...
                  <select value={formData.funded_rule_id} onChange={e => setFormData({...formData, funded_rule_id: e.target.value})} className="w-full rounded-none border border-[var(--dash-hairline)] bg-white px-3 py-2 text-sm text-ink outline-none transition-colors focus:border-ink-400 bg-white text-sm">
                    <option value="">No Rules Selected</option>
                    {rules.map(r => <option key={r.id} value={r.id}>{r.name}</option>)}
                  </select>
                </div>
              ...
              <div className="mt-6 border-t pt-4">
                ...
                {formFees.length > 0 ? (
                  <div className="space-y-4 bg-gray-50 p-4 rounded-none border">
                    <div>
                      ...
                      <select 
                        ...
                        className="w-full rounded-none border border-[var(--dash-hairline)] bg-white px-3 py-2 text-sm text-ink outline-none transition-colors focus:border-ink-400 text-sm bg-white"
                      >
                        {formFees.map((f, i) => (
                      ...
                      <div className="flex gap-4 items-end pt-2 border-t border-gray-200">
                        <div className="flex-1">
                          ...
                          <input type="number" required value={formFees[selectedFeeIndex].account_size} onChange={e => {
                            ...
                            setFormFees(newFees);
                          }} className="w-full rounded-none border border-[var(--dash-hairline)] bg-white px-3 py-2 text-sm text-ink outline-none transition-colors focus:border-ink-400 text-sm" />
                        </div>
                        <div className="flex-1">
                        ...
                        }} className="mb-0.5 p-2 text-[var(--dash-negative)] hover:bg-red-50 rounded-none">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {programs.map((prog) => (
          <div key={prog.id} className="bg-white border border-[var(--dash-hairline)] rounded-none p-6 shadow-sm relative overflow-hidden">
            <div className="flex justify-between items-start mb-4">
              <div>
                <span className="inline-flex items-center rounded-full bg-[var(--accent-50)] text-[var(--carbon-blue)] px-2.5 py-0.5 text-xs font-semibold uppercase tracking-wider mb-2">
                  {prog.key}
                </span>
              ...
              <div className="flex gap-2">
                <button onClick={() => openEditModal(prog)} className="p-2 text-gray-400 hover:text-blue-600 transition-colors bg-gray-50 rounded-none">
                  <Edit2 className="w-4 h-4" />
                </button>
              </div>
            </div>

            <div className="grid grid-cols-2 gap-4 mt-6 p-4 bg-[var(--dash-canvas)] rounded-none border border-[var(--dash-hairline)]">
              <div>
            ...
            <div className="mt-4">
              ...
              <div className="flex flex-wrap gap-2">
                {prog.tpp_program_fees?.sort((a: any, b: any) => a.account_size - b.account_size).map((f: any) => (
                  <div key={f.id} className="bg-white border border-[var(--dash-hairline)] rounded-none px-2 py-1 text-xs font-medium text-[var(--ink-700)] shadow-sm">
        ...
        {programs.length === 0 && !loading && (
          <div className="col-span-full py-12 text-center text-[var(--ink-500)] border-2 border-dashed border-[var(--dash-hairline)] rounded-none">
